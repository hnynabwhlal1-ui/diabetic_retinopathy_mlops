...
def make_gradcam_heatmap(img_array, model, last_conv_layer_name=LAST_CONV_LAYER_NAME, pred_index=None):
    ...
    handling nested sub-models (like EfficientNetB0).
    """
    # 1. Access the inner sub-model and target conv layer
    try:
        sub_model = model.get_layer("efficientnetb0")
    ...
    with tf.GradientTape() as tape:
        ...
        # Pass through remaining layers if needed
        if sub_model != model:
            x = predictions
            for layer in model.layers[model.layers.index(model.get_layer("efficientnetb0")) + 1:]:
                x = layer(x)
            final_predictions = x
        else:
            final_predictions = predictions

        # معالجة مخرج Sigmoid الثنائي (1 neuron output)
...
def overlay_heatmap(heatmap, original_img, alpha=0.4, colormap=cv2.COLORMAP_JET):
    ...
    # Convert the heatmap to RGB color space using a colormap
    heatmap_colored = np.uint8(255 * heatmap_resized)
    heatmap_colored = cv2.applyColorMap(heatmap_colored, colormap)
    heatmap_colored = cv2.cvtColor(heatmap_colored, cv2.COLOR_BGR2RGB)

    # Superimpose the heatmap onto the original image with alpha blending
